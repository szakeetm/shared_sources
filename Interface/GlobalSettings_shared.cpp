...
void GlobalSettingsBase::SMPUpdate() {
    ...
    if( time-lastUpdate>333 ) {
        ...
        for (auto& proc : procInfo.threadInfos)
        {
            ...
            processList->SetValueAt(4, i, fmt::format("[{}][{}] {}", threadStateStrings[procInfo.threadInfos[i - 2].threadState],procInfo.stepsSinceUpdate[i-2], procInfo.threadInfos[i - 2].threadStatus));
            ++i;
        }
