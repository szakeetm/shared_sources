	...
	case MSG_BUTTON:
		...
		if (src == cancelButton) {
			...
			case FACETMODE:
				...
				found = false;
				for (int i = 0; !found && i<interfGeom->GetNbFacet(); i++) {
					if (interfGeom->GetFacet(i)->selected) {
						invariant = interfGeom->GetFacet(i)->sh.center;
						found = true;
					}
				}
				break;
			case VERTEXMODE:
				...
				if (!(vertexNumber->GetNumberInt(&vertexNum)) || vertexNum<1 || vertexNum>interfGeom->GetNbVertex()) {
					GLMessageBox::Display("Invalid vertex number", "Error", GLDLG_OK, GLDLG_ICONERROR);
					return;
				}
				invariant = *(interfGeom->GetVertex(vertexNum - 1));
				break;
			default:
			...
			if (mApp->AskToReset()) {
				...
				interfGeom->ScaleSelectedVertices(invariant, factorX, factorY, factorZ, src == copyButton, work);
