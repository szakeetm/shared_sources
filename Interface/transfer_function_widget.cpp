...
void TransferFunctionWidget::draw_ui()
{
    ...
    // Crash if InvisibleButton is rendered with zero size
    if(canvas_size.x == 0.0f || canvas_size.y == 0.0f) return;
    ...
    if (clicked_on_item) {
        ...
        if (io.MouseDown[0]) {
            if (selected_point != (size_t)-1) {
                ...
                if (selected_point == 0) {
                    alpha_control_pts[selected_point].x = 0.0f;
                } else if (selected_point == alpha_control_pts.size() - 1) {
                    alpha_control_pts[selected_point].x = 1.0f;
                }
            } else {
    ...
    auto pt_clr = 0xFFFFFFFF;
    ImGuiStyle* style = &ImGui::GetStyle();
    ImVec4* colors = style->Colors;
    pt_clr = ImGui::GetColorU32(ImGuiCol_Text);
    for (const auto &pt : alpha_control_pts) {
        const vec2f pt_pos = pt * view_scale + view_offset;
        polyline_pts.push_back(pt_pos);
        draw_list->AddCircleFilled(pt_pos, point_radius, pt_clr);
    }
    draw_list->AddPolyline(
            polyline_pts.data(), (int)polyline_pts.size(), pt_clr, false, 2.f);
