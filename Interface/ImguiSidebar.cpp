    ...
    if (ImGui::Begin("[BETA] Molflow Sidebar", p_open, flags)) {
        ...
        if (ImGui::CollapsingHeader("Simulation", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::SameLine();
            {
                ...
                } else if (mApp->worker.globState.globalHits.globalHits.nbMCHit > 0) {
            ...
            ImGui::SameLine();
            if(!mApp->worker.IsRunning() && mApp->worker.globState.globalHits.globalHits.nbDesorbed > 0)
                ImGui::BeginDisabled();
            ...
            if (ImGui::BeginTable("simugrid", 2, ImGuiTableFlags_None/*ImGuiTableFlags_ScrollY | ImGuiTableFlags_SizingFixedFit |
                ...
                if ((mApp->worker.simuTimer.Elapsed() <= 2.0f) && runningState) {
                    ...
                    hit_stat = fmt::format("{} ({})",
                                           Util::formatInt(mApp->worker.globState.globalHits.globalHits.nbMCHit, "hit"),
                    ...
                    des_stat = fmt::format("{} ({})",
                                           Util::formatInt(mApp->worker.globState.globalHits.globalHits.nbDesorbed, "des"),
