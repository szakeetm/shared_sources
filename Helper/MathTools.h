...
bool    IsEqual(const doublea, const doubleb, double toleranceRatio=1E-6);
...
int LookupMomentIndex(const double key, const std::vector<std::pair<double, double>>& moments, const size_tstartIndex);
...
Vector3d
PolarToCartesian(const Vector3d &nU, const Vector3d &nV, const Vector3d &nN, const doubletheta, const doublephi,
                 const boolreverse); //sets sHandle->currentParticleTracer.direction
