...
int main(int argc, char** argv) {
    ...
    if(MFMPI::world_rank == 0){
        // Export results
        char outname[256]{};
        sprintf(outname, "out%d_%s", MFMPI::world_rank, std::filesystem::path(Settings::outputFile).filename().c_str());
        if(Settings::inputFile != outname){
            // Copy full file description first, in case outputFile is different
            printf("[%d] attempting to copy file %s to %s\n", MFMPI::world_rank, std::filesystem::relative(Settings::inputFile).c_str(), outname);
