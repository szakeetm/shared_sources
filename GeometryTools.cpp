...
std::vector<Facet*> GeometryTools::GetTriangulatedGeometry(Geometry* geometry, std::vector<size_t> facetIndices, GLProgress* prg)
{
...
std::vector<Facet*> GeometryTools::Triangulate(Facet *f) {
...
Facet* GeometryTools::GetTriangleFromEar(Facet *f, const GLAppPolygon& p, int ear) {
...
std::vector<std::vector<NeighborFacet>> GeometryTools::AnalyseNeighbors(Geometry* geometry){
    ...
    for (int i = 0; i < geometry->GetNbFacet()-1; i++) {
        Facet *f = geometry->GetFacet(i);
        for (int j = i+1; j < geometry->GetNbFacet(); j++) {
            Facet *g = geometry->GetFacet(j);
            size_t  c1,c2,l;
            if(Geometry::GetCommonEdges(f,g, &c1,&c2,&l)){
                auto angle_opt = AngleBetween2Vertices(f->sh.N, g->sh.N);
                if(!angle_opt.has_value()) {
